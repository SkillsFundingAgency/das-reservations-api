...
public class ReservationRule(Entities.Rule rule)
{
    ...
    public Course Course { get; } = new(rule.Course.CourseId, rule.Course.Title, rule.Course.Level.ToString(), rule.Course.EffectiveTo, rule.Course.ApprenticeshipType, rule.Course.LearningType);
}