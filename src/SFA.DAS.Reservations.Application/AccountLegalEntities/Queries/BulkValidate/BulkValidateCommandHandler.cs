    ...
    public class BulkValidateCommandHandler : IRequestHandler<BulkValidateCommand, BulkValidationResults>
    {
        ...
        public async Task<BulkValidationResults> Handle(BulkValidateCommand bulkRequest, CancellationToken cancellationToken)
        {
            ...
            var accountLegalEntity =
                await _accountLegalEntitiesService.GetAccountLegalEntity(bulkRequest.AccountLegalEntityId);
            ...
            if(await ApprenticeshipCountExceedsRemainingReservations(bulkRequest, result))
            {
                result.ValidationErrors.Add(new BulkValidation { Reason = $"not enough remaing reservations", FileLevelError = true });
            }
            ...
            if (await FailedAccountRuleValidation(bulkRequest.AccountId))
            {
            ...
            //    if (!await ValidateStartDate(bulkRequest.AccountId, request.StartDate.Value))
            //    {
